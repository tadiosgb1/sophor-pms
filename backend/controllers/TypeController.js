...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const { rows, count } = await Type.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Type.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      ...
      const data = await Type.create(body);
      res.json(data);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async update(req, res) {
    try {
      ...
      await Type.update(body, { where: { id: req.params.id } });
      const updated = await Type.findByPk(req.params.id);
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
