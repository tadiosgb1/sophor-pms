...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const { rows, count } = await SiteImage.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await SiteImage.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      ...
      const data = await SiteImage.create(body);
      res.json(data);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async update(req, res) {
    try {
      const body = { ...req.body };
      if (req.files && req.files['image_url']) body.image_url = req.files['image_url'][0].path;
      await SiteImage.update(body, { where: { id: req.params.id } });
      const updated = await SiteImage.findByPk(req.params.id);
      res.json(updated);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async delete(req, res) {
    try {
      await SiteImage.destroy({ where: { id: req.params.id } });
