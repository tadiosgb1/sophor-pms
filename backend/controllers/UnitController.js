...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const include = [db.Type, db.Site, db.User, db.User, db.User, db.User, db.User].filter(Boolean);
      const findOptions = { where, order, offset, limit: page_size };
      if (include.length) findOptions.include = include;

      const { rows, count } = await Unit.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Unit.findByPk(req.params.id, opts);
