...
module.exports = {
  async getAll(req, res) {
    try {
      ...
      const searchableFields = ["rent_period", "status"];
      ...
      const include = [db.Unit, db.Site, db.User, db.User, db.User, db.User].filter(Boolean);
      const findOptions = { where, order, offset, limit: page_size };
      if (include.length) findOptions.include = include;

      const { rows, count } = await Rent.findAndCountAll(findOptions);
  ...
  async getOne(req, res) {
    try {
      ...
      const data = await Rent.findByPk(req.params.id, opts);
  ...
  async create(req, res) {
    try {
      ...
      const data = await Rent.create(body);
      res.json(data);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
  ...
  async delete(req, res) {
    try {
      await Rent.destroy({ where: { id: req.params.id } });
