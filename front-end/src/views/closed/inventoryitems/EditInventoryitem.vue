    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg text-sm overflow-hidden">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="font-semibold text-gray-800">Edit Inventory Item</h2>
            <p class="text-xs text-gray-400">Update the item details below</p>
          </div>
        </div>
      ...
      <form @submit.prevent="submitForm" class="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div v-if="form.quantity && form.price_per_unit"
          class="flex items-center justify-between bg-blue-50 border border-blue-100 rounded-lg px-4 py-2.5">
          <span class="text-xs text-blue-600 font-medium">Estimated Total Value</span>
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$apiPut('/inventoryitem', this.data.id, this.form);
        this.$emit('saved');
        this.$emit('close');
      } catch (e) { console.error(e); }
