...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800">
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Income Overview</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Field1</th>
              <th class="px-6 py-3 text-left">Field2</th>
              <th class="px-6 py-3 text-left">Field3</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap font-semibold text-green-700">{{ item.Field1 }}</td>
              <td class="px-6 py-4 whitespace-nowrap font-semibold text-green-700">{{ item.Field2 }}</td>
              <td class="px-6 py-4 whitespace-nowrap font-semibold text-green-700">{{ item.Field3 }}</td>
            ...
            <tr v-if="items.length === 0">
              <td :colspan="5" class="text-center py-6 text-gray-400 italic">No Income found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <addIncome v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <editIncome v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
  </div>
</template>
...
export default {
  components: { AddIncome, EditIncome },
  data() {
    return {
      items: [],
      showModal: false,
  ...
  methods: {
    async fetchItems() {
      try { this.items = (await this.$apiGet('/income')).data || []; }
    ...
    async deleteItem(id) { 
      if(confirm('Are you sure?')) { 
        try { await this.$apiDelete(`/income/${id}/`); this.fetchItems(); } 
