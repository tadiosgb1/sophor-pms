...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading Amenity..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Amenity</h1>
      <button @click="openAddModal" class="bg-primary hover:bg-primary text-white px-4 py-2 rounded-lg font-medium shadow-md flex items-center space-x-1 text-sm">
    ...
    <!-- Add/Edit Modal -->
    <add-amenity v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <edit-amenity v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
<script>
import AddAmenity from "./AddAmenity.vue";
import EditAmenity from "./EditAmenity.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddAmenity, EditAmenity, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/amenity', params);
    ...
    async confirmDelete() {
      ...
      if(res) {
        this.$root.$refs.toast.showToast('Amenity deleted successfully', 'success');
      }
