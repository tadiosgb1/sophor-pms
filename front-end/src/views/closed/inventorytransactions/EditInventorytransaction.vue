  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm">
      <div class="flex justify-between items-center mb-4 border-b pb-2">
        <h2 class="text-lg font-semibold text-gray-800 ">Edit Inventorytransaction </h2>
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div>
         ...
         <div>
          ...
          <input v-model="form.inventory_item_id" type="text" required class="border border-gray-300 rounded-lg px-4 py-2 text-sm w-full sm:max-w-xs focus:outline-none focus:ring-2 focus:ring-green-500 shadow-sm transition duration-150" />
        </div>
  ...
  data() {
    return {
      form: {
        ...
        inventory_item_id:this.data?.inventory_item_id || '',
        note: this.data?.note || '',
