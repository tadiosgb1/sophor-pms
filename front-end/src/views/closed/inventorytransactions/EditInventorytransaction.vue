    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg text-sm overflow-hidden">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="font-semibold text-gray-800">Edit Transaction</h2>
            <p class="text-xs text-gray-400">Update the transaction details</p>
          </div>
        </div>
...
export default {
  props: { data: Object },
  data() {
    const rawDate = this.data?.transaction_date;
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$apiPut('/inventorytransaction', this.data.id, this.form);
        this.$emit('saved');
        this.$emit('close');
      } catch (e) { console.error(e); }
