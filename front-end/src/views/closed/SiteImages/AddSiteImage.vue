    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md text-sm overflow-hidden">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 rounded-lg bg-green-100 flex items-center justify-center text-green-500">
            <i class="fas fa-image text-sm"></i>
          </div>
          <div>
            <h2 class="font-semibold text-gray-800">Add Site Image</h2>
            <p class="text-xs text-gray-400">Upload a photo for this site</p>
          </div>
        </div>
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        fd.append('site_id',    this.data?.site_id || '');
        ...
        await this.$apiPost('/siteimage', fd, { 'Content-Type': 'multipart/form-data' });
        this.$emit('saved');
        this.$emit('close');
