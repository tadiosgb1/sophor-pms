...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading SiteImage..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">SiteImage</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Site_id</th><th class="px-6 py-3 text-left">Image</th><th class="px-6 py-3 text-left">Owner_id</th><th class="px-6 py-3 text-left">Created_by</th><th class="px-6 py-3 text-left">Updated_by</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.site_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.image }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.owner_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.created_by }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.updated_by }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">SiteImage #{{ index + 1 }}</h2>
    ...
    <!-- Add/Edit Modal -->
    <AddSiteImage v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditSiteImage v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>

    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete SiteImage"
      message="Are you sure you want to delete this SiteImage?"
...
<script>
import AddSiteImage from "./AddSiteImage.vue";
import EditSiteImage from "./EditSiteImage.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddSiteImage, EditSiteImage, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/siteimage', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'SiteImage-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/siteimage', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('SiteImage deleted successfully', 'success');
      }
