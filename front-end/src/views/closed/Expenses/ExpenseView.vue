...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800">
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Expense Overview</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-if="items.length === 0">
              <td :colspan="5" class="text-center py-6 text-gray-400 italic">No Expense found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <!-- Add/Edit Modal -->
    <addExpense v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <editExpense v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    async fetchItems() {
      try { this.items = (await this.$apiGet('/expense')).data || []; }
    ...
    async deleteItem(id) { 
      if(confirm('Are you sure?')) { 
        try { await this.$apiDelete(`/expense/${id}/`); this.fetchItems(); } 
