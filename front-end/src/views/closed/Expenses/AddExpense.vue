  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 text-sm transform transition-all duration-300">
      ...
      <form @submit.prevent="submitForm" class="space-y-4">
        ...
        <div class="mb-2">
          <label class="block mb-1 text-sm font-medium text-gray-700">Field1</label>
          <input 
            v-model="form.Field1" 
            type="text" 
            required
        ...
        <div class="mb-2">
          <label class="block mb-1 text-sm font-medium text-gray-700">Field2</label>
          <input 
            v-model="form.Field2" 
            type="text" 
            required
        ...
        <div class="mb-2">
          <label class="block mb-1 text-sm font-medium text-gray-700">Field3</label>
          <input 
            v-model="form.Field3" 
            type="text" 
            required
        ...
        <div class="flex justify-end gap-3 pt-2">
          ...
          <button type="submit" class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg shadow-md transition duration-150 text-sm font-medium">Add</button>
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async submitForm() {
      try {
        if ('Add' === 'Add') {
          await this.$apiPost('/expense', this.form);
        } else {
          await this.$apiPatch(`/expense/${this.data.id}/`, this.form);
        }
        this.$emit('saved');
        this.$emit('close');
