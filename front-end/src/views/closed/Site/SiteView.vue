...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    ...
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">Site</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Name</th><th class="px-6 py-3 text-left">Description</th><th class="px-6 py-3 text-left">Site_type</th><th class="px-6 py-3 text-left">Status</th><th class="px-6 py-3 text-left">Latitude</th><th class="px-6 py-3 text-left">Longitude</th><th class="px-6 py-3 text-left">Country</th><th class="px-6 py-3 text-left">State</th><th class="px-6 py-3 text-left">City</th><th class="px-6 py-3 text-left">Sub_city</th><th class="px-6 py-3 text-left">Street</th><th class="px-6 py-3 text-left">Owner_id</th><th class="px-6 py-3 text-left">Created_by</th><th class="px-6 py-3 text-left">Updated_by</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.description }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.site_type }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.status }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.latitude }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.longitude }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.country }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.state }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.city }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.sub_city }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.street }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.owner_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.created_by }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.updated_by }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">Site #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Site_type:</span>
              {{ item.site_type }}
            </div>
            <div class="col-span-2">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Sub_city:</span>
              {{ item.sub_city }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <add-site v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
...
export default {
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/site', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'Site-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/site', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('Site deleted successfully', 'success');
      }
