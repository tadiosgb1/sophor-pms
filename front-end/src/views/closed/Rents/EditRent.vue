    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg text-sm overflow-hidden">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="font-semibold text-gray-800">Edit Rental</h2>
            <p class="text-xs text-gray-400">Update the rental agreement details</p>
          </div>
        </div>
          ...
          <div class="grid grid-cols-3 gap-2">
            <button v-for="p in periodOptions" :key="p.value" type="button"
              ...
              :class="['py-2 rounded-xl border-2 transition-all text-xs font-medium',
                form.rent_period === p.value ? 'border-blue-400 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-500 hover:border-gray-300']">
              {{ p.label }}
            </button>
          </div>
        </div>
...
export default {
  props: { data: Object },
  data() {
    const toDate = ts => ts ? new Date(ts).toISOString().split('T')[0] : '';
    return {
      form: {
        ...
        start_date:     toDate(this.data?.start_date),
        next_due_date:  toDate(this.data?.next_due_date),
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$apiPut('/rent', this.data.id, this.form);
