    ...
    >
      <div class="bg-white max-w-lg w-full rounded-xl shadow-2xl overflow-hidden relative transform transition-all duration-300 border border-gray-200">
        ...
        <div class="px-6 py-4 border-b border-green-200 flex justify-between items-center">
          <h2 class="text-xl font-semibold text-gray-800">Update Group: {{ form.name }}</h2>
...
          // Ensure permissions are correctly copied for mutation
          this.form.permissions = Array.isArray(newVal.permissions) ? [...newVal.permissions] : [];
        }
      }
    }
  },
...
      } catch (err) {
        console.error(err);
        this.$root.$refs.toast.showToast('Failed to update group.', 'error');
      }
    }
  }
};
