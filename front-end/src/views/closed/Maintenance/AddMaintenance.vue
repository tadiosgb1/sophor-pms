    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg text-sm overflow-hidden">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="font-semibold text-gray-800">New Maintenance Request</h2>
            <p class="text-xs text-gray-400">Submit a maintenance or repair request</p>
          </div>
        </div>
      ...
      <form @submit.prevent="submitForm" class="px-6 py-5 space-y-4 max-h-[72vh] overflow-y-auto">
        ...
        <div>
          ...
          <input v-model="form.title" type="text" required placeholder="e.g. Leaking pipe in bathroom"
            class="form-input" />
        </div>
        ...
        <div>
          ...
          <textarea v-model="form.description" rows="3" placeholder="Describe the issue in detail…"
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div>
            <label class="form-label">Unit <span class="text-xs text-gray-400">(optional)</span></label>
        ...
        <!-- Notes -->
        <div>
          <label class="form-label">Internal Notes <span class="text-xs text-gray-400">(optional)</span></label>
          <textarea v-model="form.notes" rows="2" placeholder="Notes for staff/manager…"
            class="form-input resize-none"></textarea>
        </div>
      ...
      <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-100 bg-gray-50">
        ...
        <button @click="submitForm" :disabled="submitting || !form.category || !form.priority"
...
export default {
  ...
  data() {
    return {
      form: {
        ...
        site_id: '', unit_id: '', notes: '',
        requested_by: parseInt(localStorage.getItem('userId')) || null,
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        await this.$apiPost('/maintenance', this.form);
