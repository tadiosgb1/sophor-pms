...
<template>
  <div class="p-6 bg-gray-50 min-h-screen text-sm text-gray-800 relative">
    <!-- Loading -->
    <Loading :visible="loading" message="Loading SaleDocument..." />

    <!-- Page Header -->
    <div class="flex items-center justify-between mb-6 border-b pb-4 border-gray-200">
      <h1 class="text-lg font-bold text-gray-800">SaleDocument</h1>
        ...
        <table class="min-w-full text-sm divide-y divide-gray-200">
          <thead class="bg-gray-100 text-gray-700 uppercase text-xs font-semibold">
            <tr>
              ...
              <th class="px-6 py-3 text-left">Sale_id</th><th class="px-6 py-3 text-left">Document_name</th><th class="px-6 py-3 text-left">Document_url</th><th class="px-6 py-3 text-left">Owner_id</th><th class="px-6 py-3 text-left">Created_by</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(item, index) in items" :key="item.id" class="hover:bg-green-50 transition duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">{{ item.sale_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.document_name }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.document_url }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.owner_id }}</td><td class="px-6 py-4 whitespace-nowrap">{{ item.created_by }}</td>
    ...
    <div class="md:hidden space-y-4">
      <div v-for="(item, index) in items" :key="item.id" class="bg-white border border-gray-200 rounded-xl shadow p-4">
        <div class="flex justify-between mb-3">
          <h2 class="font-bold text-gray-800">SaleDocument #{{ index + 1 }}</h2>
        ...
        <div class="grid grid-cols-2 gap-y-1 text-sm text-gray-700">
            ...
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Sale_id:</span>
              {{ item.sale_id }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Document_name:</span>
              {{ item.document_name }}
            </div>
            <div class="col-span-2">
              <span class="font-medium text-gray-600">Document_url:</span>
              {{ item.document_url }}
            </div>
            <div class="col-span-2">
    ...
    <!-- Add/Edit Modal -->
    <AddSaleDocument v-if="showModal && !editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <EditSaleDocument v-if="showModal && editMode" :data="selectedItem" @close="showModal=false" @saved="fetchItems"/>
    <!-- Delete Confirmation Modal -->
    <delete-confirm-modal 
      :visible="deleteModalVisible"
      title="Delete SaleDocument"
      message="Are you sure you want to delete this SaleDocument?"
...
<script>
import AddSaleDocument from "./AddSaleDocument.vue";
import EditSaleDocument from "./EditSaleDocument.vue";
import Loading from "@/components/Loading.vue";
import DeleteConfirmModal from "@/components/DeleteConfirmModal.vue";

export default {
  components: { AddSaleDocument, EditSaleDocument, Loading, DeleteConfirmModal },
  ...
  methods: {
    async fetchItems(page = 1) {
      ...
      try {
        const response = await this.$apiGet('/saledocument', params);
    ...
    viewDetails(id) { 
      this.$router.push({ name: 'SaleDocument-detail', params: { id } });
    },
    ...
    // Delete with toast
    async confirmDelete() {
      const res = await this.$apiDelete('/saledocument', this.deleteId);
      if(res) {
        this.$root.$refs.toast.showToast('SaleDocument deleted successfully', 'success');
      }
